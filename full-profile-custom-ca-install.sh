...
kapp deploy \
  ...
  --namespace tap-install \
  --file <(\
     ytt --ignore-unknown-comments -f values.yaml -f ${script_dir}/ingress-config/custom-ca-issuer \
  ) \
  --yes
...
kapp deploy \
  ...
  --namespace tap-install \
  --file <(\
     ytt --ignore-unknown-comments -f values.yaml -f ${script_dir}/ingress-config/certificates-custom-ca \
  ) \
  --yes
